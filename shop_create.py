...
if not (os.path.isfile(f"./database/{shop_name}.db")):
    ...
    con.commit()
    cur.execute("""CREATE TABLE "custom_price" ("product_id" TEXT, "user_id" INTEGER, "price_1" INTEGER, "price_2" INTEGER, "price_3" INTEGER, FOREIGN KEY("product_id") REFERENCES "products"("item_id"), FOREIGN KEY("user_id") REFERENCES "users"("id"));""")
    con.commit()
